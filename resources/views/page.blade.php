...
@section('adminlte_js')
    ...
    @stack('js')
    @yield('js') //이런식으로 정의되있으니까 스크립트를 이걸로 감싸주면 이 위치에 오게 돼
    그럼 제이쿼리를 불러온 후 실행할 수 있게 되는 거지
    먼소리진잘모르겠지만
    결국문제가뭐였던겅ㅁ
    제이쿼리를 불러오기 전에 스크립트르 쓰니까 실행이 안된 거거든
    이 위치는 제이쿼리르 ㅂ불러온 다음 이니까 여기에 스크립트르 넣어야 하는데
    그럴려면 이 일드 메소드를 참조해서
    일드 메소드의 이름이 js 이니까
